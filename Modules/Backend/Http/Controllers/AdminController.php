        ...
        if ($request->isMethod('post')) {
            try {
                ...
                if ($admin_create){
                    ...
                    // $admin_create->roles->attach($roles);
                }
                \DB::commit();
                return redirect()->route('index.admin')->with('success', 'Nó đã làm việc, Đăng bài thành công!');
            } catch (\Exception $e) {
                \DB::rollBack();
                return redirect()->route('index.admin')->with('error', 'Đã có lỗi sảy ra, Đăng bài không thành công!');
            }
        }
    }
